...
export function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f5f5f7] text-[#1d1d1f] selection:bg-indigo-500/20 selection:text-indigo-900">
      ...
      {/* Hero Section */}
      <div className="relative border-b border-black/[0.06] bg-white pt-6 pb-6 sm:pt-9 sm:pb-7 overflow-hidden">
        {/* Subtle Ambient Background Gradient Light */}
        <div className="absolute top-0 right-0 w-[500px] h-[350px] bg-gradient-to-bl from-indigo-200/20 via-purple-100/10 to-transparent pointer-events-none rounded-full blur-3xl -z-0" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
            ...
            <div className="lg:col-span-7 flex flex-col justify-between space-y-4">
              <div className="space-y-3">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50/90 border border-indigo-200/80 text-xs font-mono text-indigo-700 shadow-2xs">
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-600" />
                  <span>1,000+ 精选收录 · 真实来源溯源</span>
                </div>
                
                <div>
                  <h1 className="font-hero-serif text-4xl sm:text-5xl lg:text-[54px] font-bold tracking-tight text-[#111113] leading-[1.12]">
                    Prompt <span className="font-serif font-normal italic text-[#111113]">&</span> Skill 风格大赏
                  </h1>
                </div>

                <p className="text-xs sm:text-sm md:text-base text-[#6e6e73] leading-relaxed max-w-xl font-sans">
                ...
                <div className="flex items-center gap-3 sm:gap-4.5 pt-1 text-left flex-wrap">
                  <div className="flex items-baseline gap-1">
                    <span className="text-xl sm:text-2xl font-bold font-mono text-[#111113] leading-none">{skills.length}</span>
                    <span className="text-xs text-[#86868b] font-medium font-sans">条目</span>
                  </div>
                  <div className="w-[1px] h-4 bg-black/[0.1]" />
                  <div className="flex items-baseline gap-1">
                    <span className="text-xl sm:text-2xl font-bold font-mono text-rose-600 leading-none">{typeCounts.style || 0}</span>
                    <span className="text-xs text-[#86868b] font-medium font-sans">视觉风格</span>
                  </div>
                  <div className="w-[1px] h-4 bg-black/[0.1]" />
                  <div className="flex items-baseline gap-1">
                    <span className="text-xl sm:text-2xl font-bold font-mono text-emerald-600 leading-none">{(typeCounts.skill || 0) + (typeCounts.tool || 0)}</span>
                    <span className="text-xs text-[#86868b] font-medium font-sans">开源 Skill</span>
                  </div>
                  <div className="w-[1px] h-4 bg-black/[0.1]" />
                  <div className="flex items-baseline gap-1">
                    <span className="text-xl sm:text-2xl font-bold font-mono text-indigo-600 leading-none">{uniqueAuthorsCount}+</span>
                    <span className="text-xs text-[#86868b] font-medium font-sans">真实作者</span>
                  </div>
                </div>
              </div>
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2.5 p-3 rounded-2xl transition-all duration-200 text-left border ${
                          isActive
                            ...
                            : 'bg-white hover:bg-[#fafafc] border-black/[0.08] text-[#1d1d1f] shadow-2xs hover:border-black/[0.14]'
                        }`}
                      >
                        ...
                        <div className="min-w-0">
                          <div className={`text-xs font-bold truncate ${isActive ? 'text-white' : 'text-[#1d1d1f]'}`}>
                            {t.label}
                          </div>
                          <div className={`text-xs font-mono font-medium ${isActive ? 'text-white/70' : 'text-[#86868b]'}`}>
                            {t.count}
                          </div>
                        </div>
                      </button>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-2.5 sm:px-4 md:px-6 lg:px-8 py-5 sm:py-8">
        ...
        {/* Results summary bar */}
        <div className="flex items-center justify-between mb-4 sm:mb-6 text-[11px] sm:text-xs text-[#86868b] px-1">
          <div className="flex items-center gap-1.5">
            <span>展示</span>
            <strong className="text-[#1d1d1f] font-mono font-semibold">{displayedSkills.length}</strong>
            <span>/</span>
            <span>共 {filteredSkills.length} 条</span>
            {isFiltered && <span className="text-[#86868b] hidden sm:inline">（已应用筛选）</span>}
          </div>
          {isAdmin && (
            <div className="text-emerald-700 font-medium flex items-center gap-1.5 bg-emerald-50 px-2.5 py-0.5 sm:py-1 rounded-full border border-emerald-200 text-[10px] sm:text-xs">
        ...
        {loading && (
          <div className="py-24 text-center space-y-3">
            ...
            <div className="text-xs text-[#86868b]">正在载入数据...</div>
          </div>
        )}
        ...
        {!loading && filteredSkills.length === 0 && (
          <div className="py-20 text-center space-y-3 max-w-sm mx-auto">
            <div className="w-12 h-12 rounded-2xl bg-white border border-black/[0.08] flex items-center justify-center mx-auto text-[#86868b] shadow-2xs">
              <SearchX className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-[#1d1d1f]">没有找到匹配结果</h3>
              <p className="text-xs text-[#86868b] mt-1">请尝试更换关键词或重置筛选分类。</p>
            </div>
            <button
              onClick={handleResetFilters}
              className="px-4 py-2 rounded-xl bg-[#1d1d1f] hover:bg-black text-white text-xs font-semibold shadow-xs transition-all"
            >
              清空所有筛选
            </button>
          </div>
      ...
      {/* Footer */}
      <footer className="border-t border-black/[0.06] bg-white py-8 text-center text-xs text-[#86868b]">
        <div className="max-w-7xl mx-auto px-4 space-y-1.5">
          <div className="flex items-center justify-center gap-2">
            <span className="text-[#1d1d1f] font-semibold">Prompt & Skill 风格大赏</span>
            <span>•</span>
            <span>致敬开源创作者</span>
          </div>
          ...
          <div className="pt-2 flex items-center justify-center gap-2 text-[10px] text-black/20">
            <span>© 2026 Edition</span>
            <button
              onClick={() => setIsAuthModalOpen(true)}
              className="hover:text-black/60 transition-colors p-1"
