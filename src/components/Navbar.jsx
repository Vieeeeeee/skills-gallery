      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between h-14 sm:h-16 gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl sm:rounded-2xl bg-gradient-to-b from-[#1d1d1f] to-black border border-black/10 dark:border-white/10 flex items-center justify-center shadow-xs text-white relative overflow-hidden group shrink-0">
          ...
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            <button
              onClick={onShuffle}
              className="flex items-center justify-center w-7 h-7 sm:w-auto sm:h-auto sm:px-3 sm:py-1.5 rounded-xl text-xs font-medium bg-white dark:bg-[#141417] border border-black/[0.08] dark:border-white/[0.08] text-[#515154] dark:text-zinc-300 hover:border-black/[0.15] dark:hover:border-white/[0.18] hover:text-[#1d1d1f] dark:hover:text-white shadow-2xs transition-all cursor-pointer"
              title="随机打乱排序 / 换一批灵感"
            >
              <Shuffle className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
              <span className="hidden md:inline sm:ml-1.5">换一批</span>
            </button>
        ...
        {/* Mobile Search Input (Visible only on < md screens, dedicated full-width row) */}
        <div className="md:hidden pb-2.5 pt-0.5">
          <div className="relative flex items-center w-full">
            <Search className="w-3.5 h-3.5 text-[#86868b] dark:text-zinc-500 absolute left-3.5 pointer-events-none" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="搜索风格、Skill、作者...（按 / 聚焦）"
              className="w-full pl-9 pr-9 py-1.5 bg-[#f5f5f7]/90 dark:bg-[#141417] hover:bg-[#efeff2] dark:hover:bg-[#1a1a1e] focus:bg-white dark:focus:bg-[#18181c] border border-black/[0.08] dark:border-white/[0.1] focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20 rounded-full text-xs text-[#1d1d1f] dark:text-white placeholder-[#86868b] dark:placeholder-zinc-500 transition-all outline-none shadow-2xs min-w-0"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-2.5 p-1 text-[#86868b] hover:text-[#1d1d1f] dark:text-zinc-400 dark:hover:text-white rounded cursor-pointer"
                title="清空"
              >
                <X className="w-3.5 h-3.5" />
              </button>
